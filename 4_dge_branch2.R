...
### Figure Xa - plot branch point 2
branch <- "branch2"
base_clus <- "5"
divergent_clus <- c("17","8")
...
umapHighLight <- formatUMAP(pi) + theme(axis.title = element_blank(),
...
features <- c("SLPI","CAMP","TREM1", "LGALS1") #only works with 1 feat rn
...
p <- ggplot(data = df, aes(x = ptime, y = avg, color = feat, linetype = cluster)) + theme_classic() +
geom_smooth(se = FALSE)  + labs(x = "Pseudotime", y = "Log2 normalized count") + guides(colour = guide_legend(ncol = 3)) 
ggsave(paste0("../output/", outName, "/", outName, "_feats_byTime.png"), width = 6, height = 4)
